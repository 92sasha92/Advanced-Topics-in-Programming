#include "RSPPlayer_222.h"

REGISTER_ALGORITHM(222)
RSPPlayer_222::RSPPlayer_222(): player(0), opponent(0), opponentNumOfFlags(RPS::F), opponentNumOfUnknownPieces(0), isOpponentAttacked(false), lastMove(), selfGameBoard() {
    srand (time(NULL));
...
void RSPPlayer_222::getInitialPositions(int player, std::vector<unique_ptr<PiecePosition>>& vectorToFill) {
    this->player = player;
    if (player == 1) {
        this->opponent = 2;
    } else {
...
void RSPPlayer_222::notifyOnInitialBoard(const Board& b, const std::vector<unique_ptr<FightInfo>>& fights) {
...
void RSPPlayer_222::notifyOnOpponentMove(const Move& move) {
...
void RSPPlayer_222::notifyFightResultWhenPlayerDefend(const FightInfo& fightInfo, const MyPoint &fLastPoint, const MyPoint &toLastPoint) {
    vector<unique_ptr<Piece>> trash;
...
void RSPPlayer_222::notifyFightResultWhenPlayerAttack(const FightInfo& fightInfo, const MyPoint &fLastPoint, const MyPoint &toLastPoint) {
    vector<unique_ptr<Piece>> trash;
    Piece::RPSPiecesTypes opponentType;
...
void RSPPlayer_222::notifyFightResult(const FightInfo& fightInfo) {
    vector<unique_ptr<Piece>> trash;
...
void RSPPlayer_222::setPieceData(int row, int col , bool &playerHaveFlag, bool &playerHaveMovingPieces) {
...
EndOfGameHandler RSPPlayer_222::checkWinner(EndOfGameHandler& endOfGameHandler, int currentPlayer) {
...
int RSPPlayer_222::scoringFunction(int curPlayer) {
...
int RSPPlayer_222::getPieceScore(unique_ptr<Piece> &piece) const{
    switch (piece->type) {
        case Piece::Flag:
            return FLAG_SCORE;
        case Piece::Bomb:
...
int RSPPlayer_222::getUnknownPieceTypeScore() const{
    return (FLAG_SCORE * opponentNumOfFlags)/opponentNumOfUnknownPieces;
}

int RSPPlayer_222::swapTurn(int curPlayer) {
    if (curPlayer == 1) {
        return 2;
    }
    return 1;
...
void RSPPlayer_222::undoMove(MyMove &lastMove, unique_ptr<Piece>& fromPiece, unique_ptr<Piece>& toPiece) {
...
unique_ptr<MyFightInfo> RSPPlayer_222::fight(unique_ptr<PiecePosition> &player1PiecePos, unique_ptr<PiecePosition> &player2PiecePos) {
...
unique_ptr<MyFightInfo> RSPPlayer_222::makeMove(unique_ptr<Move> &pieceMove, int player) {
...
bool RSPPlayer_222::indexCheck(int row, int col) {
...
void RSPPlayer_222::setPointAndGetScore(int row, int col, MyPoint &pTo, MyMove &curMove, int curPlayer, int depth, bool isMax, int &bestScore) {
    int curScore;
...
int RSPPlayer_222::recFuncHandler(MyMove &curMove,  int curPlayer, int depth, bool isMax) {
...
void RSPPlayer_222::handleOneOfTheMoveChoice(int row, int col, MyPoint &pTo, MyPoint &bestPFrom, MyPoint &bestPTo, int curPlayer, int &curScore, int &bestScore, MyMove &curMove, int depth, bool isMax, vector<unique_ptr<Move>> &bestMovesVec) {
...
unique_ptr<Move> RSPPlayer_222::getMove() {
    bool isMax = true;
...
int RSPPlayer_222::getJokerMoveScore(int row, int col, unique_ptr<Piece> &tmpPiece) {
...
int RSPPlayer_222::getScoreForJokerRep(int row, int col, Piece::RPSJokerTypes jokerRep) {
    int score = 0;
...
unique_ptr<JokerChange> RSPPlayer_222::getJokerChange() {
