#include "RSPPlayer_323239566.h"

REGISTER_ALGORITHM(323239566)
RSPPlayer_323239566::RSPPlayer_323239566(): player(0), opponent(0), opponentNumOfFlags(RPS::F), opponentNumOfUnknownPieces(0), isOpponentAttacked(false), lastMove(), selfGameBoard() {
    srand (time(NULL));
...
void RSPPlayer_323239566::getInitialPositions(int player, std::vector<unique_ptr<PiecePosition>>& vectorToFill) {
    this->player = player;
    if (player == 1) {
        this->opponent = 2;
    } else {
...
void RSPPlayer_323239566::notifyOnInitialBoard(const Board& b, const std::vector<unique_ptr<FightInfo>>& fights) {
...
void RSPPlayer_323239566::notifyOnOpponentMove(const Move& move) {
...
void RSPPlayer_323239566::notifyFightResultWhenPlayerDefend(const FightInfo& fightInfo, const MyPoint &fLastPoint, const MyPoint &toLastPoint) {
    vector<unique_ptr<Piece>> trash;
...
void RSPPlayer_323239566::notifyFightResultWhenPlayerAttack(const FightInfo& fightInfo, const MyPoint &fLastPoint, const MyPoint &toLastPoint) {
    vector<unique_ptr<Piece>> trash;
    Piece::RPSPiecesTypes opponentType;
...
void RSPPlayer_323239566::notifyFightResult(const FightInfo& fightInfo) {
    vector<unique_ptr<Piece>> trash;
...
void RSPPlayer_323239566::setPieceData(int row, int col , bool &playerHaveFlag, bool &playerHaveMovingPieces) {
...
EndOfGameHandler RSPPlayer_323239566::checkWinner(EndOfGameHandler& endOfGameHandler, int currentPlayer) {
...
int RSPPlayer_323239566::scoringFunction(int curPlayer) {
...
int RSPPlayer_323239566::getPieceScore(unique_ptr<Piece> &piece) const{
    switch (piece->type) {
        case Piece::Flag:
            return FLAG_SCORE;
        case Piece::Bomb:
...
int RSPPlayer_323239566::getUnknownPieceTypeScore() const{
    return (FLAG_SCORE * opponentNumOfFlags)/opponentNumOfUnknownPieces;
}

int RSPPlayer_323239566::swapTurn(int curPlayer) {
    if (curPlayer == 1) {
        return 2;
    }
    return 1;
...
void RSPPlayer_323239566::undoMove(MyMove &lastMove, unique_ptr<Piece>& fromPiece, unique_ptr<Piece>& toPiece) {
...
unique_ptr<MyFightInfo> RSPPlayer_323239566::fight(unique_ptr<PiecePosition> &player1PiecePos, unique_ptr<PiecePosition> &player2PiecePos) {
...
unique_ptr<MyFightInfo> RSPPlayer_323239566::makeMove(unique_ptr<Move> &pieceMove, int player) {
...
bool RSPPlayer_323239566::indexCheck(int row, int col) {
...
void RSPPlayer_323239566::setPointAndGetScore(int row, int col, MyPoint &pTo, MyMove &curMove, int curPlayer, int depth, bool isMax, int &bestScore) {
    int curScore;
...
int RSPPlayer_323239566::recFunc(int curPlayer, int depth, bool isMax) { // TODO: give bonus in the scoring function if they eat piece in early move (high depth)
    EndOfGameHandler endOfGameHandler;
    checkWinner(endOfGameHandler, curPlayer);
...
int RSPPlayer_323239566::recFuncHandler(MyMove &curMove,  int curPlayer, int depth, bool isMax) {
...
void RSPPlayer_323239566::handleOneOfTheMoveChoice(int row, int col, MyPoint &pTo, MyPoint &bestPFrom, MyPoint &bestPTo, int curPlayer, int &curScore, int &bestScore, MyMove &curMove, int depth, bool isMax, vector<unique_ptr<Move>> &bestMovesVec) {
...
unique_ptr<Move> RSPPlayer_323239566::getMove() {
    bool isMax = true;
...
int RSPPlayer_323239566::getJokerMoveScore(int row, int col, unique_ptr<Piece> &tmpPiece) {
...
int RSPPlayer_323239566::getScoreForJokerRep(int row, int col, Piece::RPSJokerTypes jokerRep) {
    int score = 0;
...
unique_ptr<JokerChange> RSPPlayer_323239566::getJokerChange() {
...
void RSPPlayer_323239566::printBoard() {
