#include "RSPPlayer_111.h"

REGISTER_ALGORITHM(111)
RSPPlayer_111::RSPPlayer_111(): player(0), opponent(0), opponentNumOfFlags(RPS::F), opponentNumOfUnknownPieces(0), isOpponentAttacked(false), lastMove(), selfGameBoard() {
    srand (time(NULL));
...
void RSPPlayer_111::getInitialPositions(int player, std::vector<unique_ptr<PiecePosition>>& vectorToFill) {
    this->player = player;
    if (player == 1) {
        this->opponent = 2;
    } else {
...
void RSPPlayer_111::notifyOnInitialBoard(const Board& b, const std::vector<unique_ptr<FightInfo>>& fights) {
...
void RSPPlayer_111::notifyFightResultWhenPlayerDefend(const FightInfo& fightInfo, const MyPoint &fLastPoint, const MyPoint &toLastPoint) {
    vector<unique_ptr<Piece>> trash;
...
void RSPPlayer_111::notifyFightResultWhenPlayerAttack(const FightInfo& fightInfo, const MyPoint &fLastPoint, const MyPoint &toLastPoint) {
    vector<unique_ptr<Piece>> trash;
    Piece::RPSPiecesTypes opponentType;
...
void RSPPlayer_111::setPieceData(int row, int col , bool &playerHaveFlag, bool &playerHaveMovingPieces) {
...
EndOfGameHandler RSPPlayer_111::checkWinner(EndOfGameHandler& endOfGameHandler, int currentPlayer) {
...
int RSPPlayer_111::scoringFunction(int curPlayer) {
...
int RSPPlayer_111::getPieceScore(unique_ptr<Piece> &piece) const{
    switch (piece->type) {
        case Piece::Flag:
            return FLAG_SCORE;
        case Piece::Bomb:
...
int RSPPlayer_111::getUnknownPieceTypeScore() const{
    return (FLAG_SCORE * opponentNumOfFlags)/opponentNumOfUnknownPieces;
}

int RSPPlayer_111::swapTurn(int curPlayer) {
    if (curPlayer == 1) {
        return 2;
    }
    return 1;
...
void RSPPlayer_111::undoMove(MyMove &lastMove, unique_ptr<Piece>& fromPiece, unique_ptr<Piece>& toPiece) {
        MyPoint fLastPoint(lastMove.getFrom().getX(), lastMove.getFrom().getY());
    MyPoint toLastPoint(lastMove.getTo().getX(), lastMove.getTo().getY());
    vector<unique_ptr<Piece>> trash;
    trash.push_back(std::move(selfGameBoard[toLastPoint.getY()][toLastPoint.getX()]));
...
unique_ptr<MyFightInfo> RSPPlayer_111::fight(unique_ptr<PiecePosition> &player1PiecePos, unique_ptr<PiecePosition> &player2PiecePos) {
...
unique_ptr<MyFightInfo> RSPPlayer_111::makeMove(unique_ptr<Move> &pieceMove, int player) {
...
bool RSPPlayer_111::indexCheck(int row, int col) {
...
void RSPPlayer_111::setPointAndGetScore(int row, int col, MyPoint &pTo, MyMove &curMove, int curPlayer, int depth, bool isMax, int &bestScore) {
    int curScore;
...
int RSPPlayer_111::recFuncHandler(MyMove &curMove,  int curPlayer, int depth, bool isMax) {
...
void RSPPlayer_111::handleOneOfTheMoveChoice(int row, int col, MyPoint &pTo, MyPoint &bestPFrom, MyPoint &bestPTo, int curPlayer, int &curScore, int &bestScore, MyMove &curMove, int depth, bool isMax, vector<unique_ptr<Move>> &bestMovesVec) {
...
unique_ptr<Move> RSPPlayer_111::getMove() {
    bool isMax = true;
...
int RSPPlayer_111::getJokerMoveScore(int row, int col, unique_ptr<Piece> &tmpPiece) {
...
int RSPPlayer_111::getScoreForJokerRep(int row, int col, Piece::RPSJokerTypes jokerRep) {
    int score = 0;
...
unique_ptr<JokerChange> RSPPlayer_111::getJokerChange() {
