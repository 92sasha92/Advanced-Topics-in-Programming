...
static bool test3(){
    ...
    int playerNum = 1;
    char piece = 'k';
    auto iter = board.allOccureneceOfPieceForPlayer(piece, playerNum).begin();
    for(; iter != board.allOccureneceOfPieceForPlayer(piece, playerNum).end(); ++iter) {
        cout << "row: " << std::get<0>(*iter) << endl;
        cout << "col: " << std::get<1>(*iter) << endl;
        cout << "piece: " << std::get<2>(*iter) << endl;
        cout << "player: " << std::get<3>(*iter) << endl;
    }
    return true;
}

static bool test0(){
    GameBoard<0, 0, char, 4> board;
	int playerNum = 1;
//	char piece = 'k'; // we assume in this case that GAME_PIECE is char
	for(auto pieceInfo : board.allPiecesOfPlayer(playerNum)) {
		cout << "row: " << std::get<0>(pieceInfo) << endl;
        cout << "col: " << std::get<1>(pieceInfo) << endl;
        cout << "piece: " << std::get<2>(pieceInfo) << endl;
